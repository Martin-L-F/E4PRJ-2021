...
public:
	ControlUnit_Controller()
	{
		...
		server.add_onConnected_Handler(std::bind(&ControlUnit_Controller::onWebSocketConnected, this, std::placeholders::_1));
...
private:
	...
	void onWebSocketMessage(Message* message) {
		if (message->getContent()["type"] == "RequestConnectionVerification") {
			...
			if (message->messageRequiresType(Client::connectionType::pistol)) return;
			...
			if (message->messageRequiresType(Client::connectionType::primaryBrowser)) return;
	...
	void armAllPistols(std::string difficulty) {
		...
		if (res.size() > 0) {
			...
			message["frequency"]["range"] = res.size();
			int index = 0;
			for (auto& client : res) {
	...
	void onWebSocketConnected(Client* client) {

	}

	// Hadndler for disconnected WebSocket connection.
